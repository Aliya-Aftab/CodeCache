...
export default function SnippetList({ 
  ...
  // Safety check: if snippets is null/undefined, treat as empty array
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5">
      {safeSnippets.map(item => (
        <div key={item._id || Math.random()} className="group bg-slate-900 border border-slate-800 rounded-xl overflow-hidden hover:border-blue-500/50 transition-all hover:shadow-2xl hover:shadow-black/50 flex flex-col h-[280px]">
          <div className="p-4 border-b border-slate-800 flex justify-between items-start bg-slate-900/50">
            <div className="overflow-hidden">
              <h3 className="font-bold text-slate-200 group-hover:text-blue-400 truncate pr-2 transition-colors">{item.title || 'Untitled'}</h3>
              <div className="flex gap-2 mt-2">
                <span className="text-[10px] px-2 py-0.5 rounded bg-blue-500/10 text-blue-400 border border-blue-500/20 uppercase font-bold tracking-wider">{item.language || 'text'}</span>
          ...
          <div className="flex-1 bg-[#0B1120] p-4 overflow-hidden relative group-hover:bg-black transition-colors">
            <pre className="text-xs font-mono text-slate-400 leading-relaxed opacity-75">{item.code || '// No code content'}</pre>
